...
export default function DashboardLayout({
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        if (user) {
          // Compare email with dynamic env variable
  ...
  const navItems = [
    ...baseNavItems,
    ...(isAdmin ? [{ label: "Admin Access", href: "/dashboard/admin", icon: <AdminIcon /> }] : [])
  ];
  ...
  return (
    <div className="flex min-h-screen bg-surface-950">
      ...
      >
        ...
        {/* Upgrade CTA (Hide for premium/admin) */}
        {userTier !== "Premium Plan" && !isAdmin && (
        ...
        <div className="px-4 py-4 border-t border-white/5 flex items-center gap-3">
          <div className="w-8 h-8 rounded-full gradient-bg flex items-center justify-center text-xs font-bold text-white uppercase">
            {userName.charAt(0)}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate">{userName}</p>
            <p className="text-xs text-gray-500 truncate">{isAdmin ? "Administrator" : userTier}</p>
          </div>
        </div>
      </aside>
